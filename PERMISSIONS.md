...
> **2026-08-07 — Sport Admin's roster grants made reachable: NO policy change, hooks only.** The §3b reconcile of 2026-08-05 removed Sport Admin's `admin_access` (see that entry), which was correct — and it silently disarmed the collections §9 grants them. Every roster/staff scope guard in `kscw-hooks` bypassed on **`accountability.admin` alone**, so from the moment the escalation row was pruned, a `vb_admin` / `bb_admin` who does not personally coach a team was refused on `member_teams` create/update/delete and `teams_coaches` / `teams_responsibles` create — collections `SPORT_ADMIN_FULL_CRUD` grants them **club-wide**. The frontend never agreed with that: `isCoachOf` resolves to `hasAdminAccessToTeam || coach`, so the roster editor and the Data Explorer's staff columns stayed open to sport admins and every write from them failed. The guards now consult **`actorMayManageTeam`** = leads the team **OR** `actorHasClubWideTeamAccess` — `members.role ∋ admin | superuser` passes for any team; `vb_admin` / `bb_admin` pass only for a team whose `teams.sport` matches their section (both roles ⇒ every team); a team with **no `sport`** resolves to false for sport admins, the same conservative answer `hasAdminAccessToTeam` gives, so nobody gains access through missing data. `members.delete` / `teams.delete` stay withheld (see 2026-08-06) — this restores exactly the grants §9 already declares, and restores them in the hook layer rather than by handing the `admin_access` bypass back. ⚠ **Any future collection added to `SPORT_ADMIN_FULL_CRUD` that a hook guards on `accountability.admin` alone is a dead grant by the same mechanism.** Second fix in the same commit, unrelated to scope: `kscwScopeError` (and the CAPTCHA + announcement-audience throws) built a plain `Error`, which `isDirectusError()` rejects because it tests `name === 'DirectusError'` and nothing else — so Directus answered **500 "An unexpected error occurred."** and, for non-admins, discarded the message. ~25 deliberate 403s were reporting themselves as server crashes; they now carry their status, code and reason. `kscw-hooks` only — **needs `ext:deploy`, not `db:setup-perms`**; no permission row changed.
...
- `members` — create / read / update **and delete** since 2026-08-06. Delete is used by exactly one surface, the Data Explorer danger zone, and goes through `POST /kscw/admin/delete-member` (impact preview + typed `DELETE` gate in the UI; `ItemsService` with the caller's accountability + a server-side sport-scope re-check in the endpoint, which also removes the linked `directus_users` login the items API would leave behind). See the 2026-08-06 banner note at the top.
