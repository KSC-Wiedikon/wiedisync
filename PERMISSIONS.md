...
| `EVENT_ROSTER_VISIBLE` | `{ activity_type: 'event' }` `_and` `{ event: EVENTS_VISIBLE }` — "you read the RSVPs of any event you can see" (migration 333 added the `participations.event` m2o that makes this expressible) | participations (read only) |
...
| participations | `PARTICIPATION_VISIBLE` = `SAME_TEAM_AS_ME` ∪ `SAME_GAME_AS_ME` ∪ `EVENT_ROSTER_VISIBLE` | `MEMBER_PARTICIPATION_FIELDS` (all except the `last_*_edited_by` directus_users UUIDs — 2026-05-12 audit #12; incl. `auto_declined_by_game` since migration 261 and `event` since 333) | 033 / 271 / **333** |
...
| participations | read | `COACH_OR_TR_PARTICIPATION_READ` = `COACH_OR_TR_OF_PARTICIPATION` ∪ `{ activity_type: 'event' } _and { event: LEADER_EVENTS_VISIBLE }` — the whole roster of any event the leader can see | 026 / 271 / **333** |
| participations | update / delete | `COACH_OR_TR_OF_PARTICIPATION` — **deliberately narrower than read** (333): a leader sees a club event's full roster but may only edit their own people's answers | 026 |
