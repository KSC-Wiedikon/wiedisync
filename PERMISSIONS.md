...
| `EVENT_ROSTER_VISIBLE` ⚠ **PARKED 26.08.2026** | `{ activity_type: 'event' }` `_and` `{ event: { _and: [{ open_roster: true }, EVENTS_VISIBLE] } }` — the roster of a **cross-team** event you can see (333 added the `participations.event` m2o that makes it expressible, 334 the `events.open_roster` flag that keeps single-team events out) | participations (read only) |
...
| participations | `PARTICIPATION_VISIBLE` = `SAME_TEAM_AS_ME` **only** ⚠ (271 + 333/334 branches PARKED 26.08.2026 — perf stopgap, see top entry; restore) | `MEMBER_PARTICIPATION_FIELDS` (all except the `last_*_edited_by` directus_users UUIDs — 2026-05-12 audit #12; incl. `auto_declined_by_game` since migration 261 and `event` since 333) | 033 / 271 / **333 + 334** |
...
| participations | read | ⚠ `COACH_OR_TR_PARTICIPATION_READ_NARROWED` = own ∪ coach-of-active-team ∪ TR-of-active-team (26.08.2026 perf stopgap — the event-roster **and** game-guest branches are PARKED; read is now NARROWER than update/delete, inverting the 333 posture) | 026 / 271 / **333 + 334** |
