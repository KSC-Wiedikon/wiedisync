...
> **2026-08-13 — Staged FIVB player index (migration 313): ONE new read grant.** New `vis_players` collection — the VIS player index for the ~28 federations our members claim as federation of origin, mirrored so the transfer worklist's matching is answerable after the fact instead of only re-runnable. **One permission row changes**: `vis_players` joins `KSCW Sport Admin`'s read list beside `vis_transfers` / `vis_federations` (same tier, same reason — `/admin/transfers` is gated to `admin | superuser | vb_admin | bb_admin`, the first two bypass policies and the sport admins do not, so without the grant the page degrades for exactly the people it is built for). **No Member, Leader, Vorstand or Terminplanung grant**, and none is defensible: these are names of people who are not our members, carried solely to resolve our own players against a federation register. ⚠ Read-only mirror — written by the `vis_player_check` cron and its on-demand endpoint, never by the UI, and **fully replaced** on each run rather than accumulated, so a federation nobody claims any more cannot linger. The footprint is deliberately the minimum the upstream request already returns: VIS player number + first/last name + which federation listed them. **No birthdate, no club, no contact detail** — the `GetPlayerList` call asks for `Properties="No"` plus `FirstName LastName`, and the table cannot hold more than that. This changes RETENTION, not exposure: every one of these rows was already downloaded on every run and discarded. `setup-permissions.mjs` §9 Sport Admin.
>
