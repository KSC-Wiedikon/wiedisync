...
> **2026-08-26 (stage 1, same day) — the event roster comes back; the game-guest branch does not. The two were never equally expensive.** `participations` read on **`KSCW Member`** and **`KSCW Team Responsible`** regains `EVENT_ROSTER_VISIBLE` (333/334). `SAME_GAME_AS_ME` (271) stays parked. **Why the split:** measured separately on the dev clone, same 302 driving rows — full rule **148,915,476** intermediate rows (6,700×) · minus `EVENT_ROSTER_VISIBLE` **5,625,450** (254×) · minus `SAME_GAME_AS_ME` **622,853** (28×) · team-scope only **22,175** (1×). The factors MULTIPLY (28 × 254 ≈ 7,100 ≈ the observed 6,700×), so **the game-guest branch is the expensive one by an order of magnitude** and the 2026-08-22 event work — which the morning's own DEVLOG entry blamed — is the cheap one. Restoring it alone costs 28× of a very small number. **Verified on prod after the run:** Photoday Day 2 (club-wide, `event_type` `verein`) **56/56** RSVPs readable by a member on no team at all; Trainingsweekend (2 teams) — a member of ONE invited team went **40 → 84 of 84**, which is precisely the cross-team gap 333 existed to close. Request timings **0–120 ms**, zero over 2 s (stopgap was 0–70 ms; the broken rule was 1m50s–2m05s). `576 permissions, 0 errors`. ⚠ Trainingsweekend reads 0 for a teamless viewer and that is CORRECT — its `event_type` is `trainingsweekend`, not `verein`/`tournament`, so `EVENTS_VISIBLE`'s club-wide branch does not apply and it reaches only its two invited teams. ⚠ **This is still a 2-branch policy and it still multiplies.** It is acceptable because 28× of a small number is small, NOT because the shape is safe — do not add a third to-many branch on top of it. ⚠⚠ `SAME_GAME_AS_ME` must NEVER return as a filter: 254× on top of this is ~7,600× and reinstates the outage. It has to stop being a relation walk — stage 2 is `participation_visibility(participation, viewer_user)`, one branch, one hop, ~680k trigger-maintained rows, built **verifier-first** because a missed trigger source is a SILENT LEAK (strictly worse than the perf bug, since nobody notices). Design: `.planning/2026-08-26-participation-visibility-design.md`. ⚠ Leader update/delete still carry the full `COACH_OR_TR_OF_PARTICIPATION`, so the read-narrower-than-write inversion persists for game guests only. ⚠ The first prod run died mid-way on a transient Cloudflare **502** — the script clears a policy's rows before recreating them, so a fatal leaves prod **half-applied**; it was re-run immediately to `0 errors` and the per-policy row counts checked. Always re-run, always verify. `setup-permissions.mjs` §Member team-scoped reads + §7 Leader.
>
> **2026-08-26 — ⚠⚠ NARROWED UNDER DURESS: `participations` read loses the 271 and 333 branches. A PERFORMANCE STOPGAP, NOT A SECURITY DECISION — restore it.** `participations` read on **`KSCW Member`** and **`KSCW Team Responsible`** drops back to team-scope only: Member = `SAME_TEAM_AS_ME`, Leader = own ∪ coach-of-active-team ∪ TR-of-active-team. `SAME_GAME_AS_ME` (271) and `EVENT_ROSTER_VISIBLE` (333/334) are **parked as comments in `setup-permissions.mjs`**, not deleted. **Why:** Directus does not compile a policy `_or` into EXISTS subqueries — it emits one flat `LEFT JOIN` per relation hop, as **siblings** off shared parents, then re-evaluates the entire predicate inside `COUNT(CASE WHEN … )` **once per readable field** (22 fields, 707 bound parameters in the captured prod SQL). Sibling joins **cross-multiply**, so branches do not add, they multiply: **302 participation rows → 46 LEFT JOINs → 148,915,476 intermediate rows.** Measured branch factors: teammates 41×, my-team-games→guests 36×, event-roster 29×, I-am-guest 11×, coaches/TRs 3.6×. **Prod impact:** every member's home page issues this read unscoped (`useBulkParticipations`, `limit=-1`); it cost 7s uncontended and **1m50s–2m05s during the morning login peak**, with requests abandoned by the browser at 2m05s. Members saw the logo spin forever. Sustained ~2h on 26.08, ~350–400 multi-second calls/day since 22.08. ⚠ It is **not data volume and not a missing index** — 12,309 participations / 713 members / 565 games, and each branch hand-written as a subquery returns in **2–6 ms**. `work_mem` does not help (4MB→40s, 64MB→**58s**, i.e. worse); `join_collapse_limit` cannot fix a genuine 149M-row cross-product and costs ~600 ms extra *planning* per query at 16. Measured on the dev clone (identical data): all 5 branches 148,915,476 rows / 60 s · without `EVENT_ROSTER_VISIBLE` 5,625,450 / 2.9 s · without `SAME_GAME_AS_ME` 622,853 / 0.19 s · **team-scope only 22,175 / 0.012 s (~6,700× cheaper)**. Post-deploy prod timings went **25.3 s → 0–70 ms**. **What regresses (both must come back):** called-up guests can no longer read each other's game RSVPs (271), and multi-team event rosters return to the pre-Photoday gap (333) — event 27 goes back to a DU20 player reading 0 of 32 confirmations. ⚠ **Leader `update`/`delete` deliberately keep the FULL `COACH_OR_TR_OF_PARTICIPATION`** — a write filter matches one row so it costs nothing, and narrowing it would strip coaches of RSVP edits. That inverts the 333 posture: a leader can now **update a guest's game RSVP they can no longer read**. Temporary; restoring the parked rule closes it. ⚠ Narrowing a read filter only ever **removes** visibility — this cannot leak data. **The fix is not to re-widen the filter as written** but to stop expressing "whose RSVP may I see?" as repeated deep relation walks and precompute it — the same move **334** already made with the trigger-derived `events.open_roster`. `setup-permissions.mjs` §Member team-scoped reads + §7 Leader.
>
> **2026-08-22 — A MULTI-TEAM event shows its whole roster to its whole audience (migrations 333 + 334): two read rows widen, no write row moves.** `participations` read on **`KSCW Member`** and **`KSCW Team Responsible`** gains one `_or` branch — `{ activity_type: 'event' }` `_and` the *same filter object* that the policy already uses for `events` read. The rule: **you can read the RSVPs of any event you can see, provided its audience spans more than one team.** What it fixes: `participations` is polymorphic (`activity_type` + a **varchar** `activity_id`), so no Directus filter could ever join an RSVP back to its event, and every read rule here had to be written member-side — `SAME_TEAM_AS_ME`, "do this row's member and I share an active team?". That is right for trainings and games, whose audience IS one team, and it silently breaks every MULTI-TEAM event. Measured on prod before the change: event 27 "Photoday mixed tournament" — 12 teams, 214 invited, 32 confirmed — a **DU20 player could read 0 of the 32 confirmations**, an H3 player 6, nobody but an admin more than 9. The failure was invisible rather than loud: names come from `members` (unfiltered read), so all 214 rows still rendered and the missing RSVPs simply displayed as "not responded" — the page said *hardly anyone answered*, not *you may not see this*. Coaches were cut identically through `COACH_OR_TR_OF_PARTICIPATION`. Migration 271 hit the same wall for called-up players and worked around it with two more member-side branches (`SAME_GAME_AS_ME`); that trick does not scale to an audience made of teams + roles + individuals. **Migration 333 supplies the missing join instead**: `participations.event`, a real m2o to `events` kept in sync with `activity_id` by `trg_participations_sync_event`, registered in `directus_fields`/`directus_relations` and added to `MEMBER_PARTICIPATION_FIELDS`. It duplicates no truth — `(activity_type, activity_id)` stay the source and still key the unique indexes. ⚠ **The branch carries TWO conditions and the second is the point.** "Can I see the event?" alone is too loose: `EVENTS_VISIBLE` opens every `verein`/`tournament` event to the whole club so it lands in everyone's calendar, but plenty of those are one team's day out — **"Rämi Turnier" is H3 only**. Being *listed* club-wide and being *rostered* club-wide are different questions. So the roster must additionally be genuinely cross-team: **migration 334** adds `events.open_roster`, trigger-derived on both sides (`events.invited_roles` and the `events_teams` count) as `teams <> 1 OR invited_roles non-empty`. **`<> 1`, not `> 1`** — zero invited teams means club-wide (Generalversammlung, Mixed-Turnier, Photoday Day 2 carry no `events_teams` rows at all), the broadest audience there is; **one** team is the only narrow case, and it needs nothing from this branch because everyone in a single-team event's audience shares that team, so `SAME_TEAM_AS_ME` already shows them the whole roster. Roles force it true even alongside one team ("H3 + every scorer" reaches people who share no team with H3). A Directus filter cannot COUNT, which is why this is a stored column rather than an inline condition. Verified against prod data across all 12 events: event 27 goes 0–19 → **95/95 for every viewer**; the 2-team `trainingsweekend` goes 0–42 → **0–84** (both invited teams see all, outsiders still 0); and **every single-team event is byte-identical before and after** — Rämi 0–14, Wetzikon 0–9 / 0–17, Rämi-Turnier 0–13, the D4 friendly 0–20. ⚠ Free-text `note` rides along with `status` — a Directus policy has no per-branch field split. ⚠ **LEADER read is now wider than LEADER update/delete**, on purpose: `COACH_OR_TR_PARTICIPATION_READ` carries the new branch, the write rows stay on `COACH_OR_TR_OF_PARTICIPATION`. Sharing a club tournament with H3 is not a mandate over H3's answers. Both policies hand the *same object* to `events` read and to the new branch (`EVENTS_VISIBLE` / `LEADER_EVENTS_VISIBLE`, the latter hoisted out of its call site for this), so event visibility and roster visibility cannot drift apart. `absences` is untouched. `setup-permissions.mjs` §Member team-scoped reads + §7 Leader.
>
> **2026-08-20 — A leader may edit a STAFF-ONLY teammate's number + position (migration 331): one new grant row, two new aliases.** `KSCW Team Responsible` gains a **third** `members.update` row, fields `position, number`, scoped by `COACH_TEAM_STAFF`: the target is coach or team responsible of a team the caller is coach or team responsible of. It closes a gap the other two rows could not see — `COACH_TEAM_MEMBERS` walks `member_teams.team.{coach,team_responsible}` and `COACH_REQUESTED_TEAM` walks `requested_team`, and a **staff-only** person has neither: they are linked to the team through `teams_coaches` / `teams_responsibles` and hold no roster row at all (38 people across 25 active teams on the day this shipped, i.e. most teams). `TeamDetail`'s Staff section renders exactly those people — a separate `extraCoaches` fetch splices them in beside the roster staff — with the same editable number + position cells as any roster row, so every edit 403'd, and `MemberRow.saveField` swallowed the rejection, leaving the cell to snap back with no message. Reported as "I have no team and can't change my players" by D2's (volleyball) coach; the same session shows the workaround it pushes people toward — inserting a `member_teams` row for a non-playing coach, which quietly makes them a roster player for RSVP audiences and every squad count. ⚠ The grant needed **migration 331** first, because the filter was not expressible: `teams.coach` / `teams.team_responsible` name the *team* side of each junction, but the *member* side of both relations had `one_field` NULL, so there was no path from a member to the teams they are staff of. 331 names them (`members.coach_of`, `members.team_responsible_of` — metadata only, no DDL, `hidden`, mirroring `members.member_teams`'s proven `o2m`-into-the-junction shape). ⚠ **Directus caches the schema at boot** — the container must restart between the migration and `db:setup-perms:*`, or the new filter is written against aliases the permission engine cannot resolve. ⚠ Deliberately **NOT** `coach_approved_team`: approving a join request is a roster act, and `TeamDetail.handleApprove` creates the `member_teams` row *first*, which puts the target inside `COACH_TEAM_MEMBERS` anyway — the field stays on that row where the "member_teams must exist" Postgres trigger backs it up. ⚠ Deliberately **update only, no READ widening**: a coach still reads a staff-only teammate through the unfiltered `KSCW Member` row (name, photo, number, position), *not* through `LEADER_TEAM_MEMBER_FIELDS` — so this does not hand anybody a fellow coach's phone, address or birthdate. Admin and Sport Admin were never affected (`members` update, fields = `*`, no filter, §9). `setup-permissions.mjs` §Members update.
>
...
| `EVENT_ROSTER_VISIBLE` (restored 26.08.2026 stage 1) | `{ activity_type: 'event' }` `_and` `{ event: { _and: [{ open_roster: true }, EVENTS_VISIBLE] } }` — the roster of a **cross-team** event you can see (333 added the `participations.event` m2o that makes it expressible, 334 the `events.open_roster` flag that keeps single-team events out) | participations (read only) |
...
| participations | `PARTICIPATION_VISIBLE` = `SAME_TEAM_AS_ME` ∪ `EVENT_ROSTER_VISIBLE` ⚠ (271 `SAME_GAME_AS_ME` PARKED 26.08.2026 — 254× cost; restore only via stage 2, never as a filter) | `MEMBER_PARTICIPATION_FIELDS` (all except the `last_*_edited_by` directus_users UUIDs — 2026-05-12 audit #12; incl. `auto_declined_by_game` since migration 261 and `event` since 333) | 033 / 271 / **333 + 334** |
...
| members | update | scoped to my-team **staff** (`COACH_TEAM_STAFF` — the target is coach/TR of a team I am coach/TR of, via the `members.coach_of` / `members.team_responsible_of` aliases), fields = `position, number` | **331** (2026-08-20), staff-row edit path (`TeamDetail` Staff section → `MemberRow.saveField`) |
| teams | read | none | |
...
| participations | read | ⚠ `COACH_OR_TR_PARTICIPATION_READ_NARROWED` = own ∪ coach-of-active-team ∪ TR-of-active-team ∪ event-roster branch (333/334 restored 26.08.2026 stage 1; the 271 game-guest branch stays PARKED, so read is narrower than update/delete for guests only) | 026 / 271 / **333 + 334** |
| participations | update / delete | `COACH_OR_TR_OF_PARTICIPATION` — **deliberately narrower than read** (333/334): a leader sees a club event's full roster but may only edit their own people's answers | 026 |
