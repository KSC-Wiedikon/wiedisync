...
| `EVENT_ROSTER_VISIBLE` | `{ activity_type: 'event' }` `_and` `{ event: { _and: [{ open_roster: true }, EVENTS_VISIBLE] } }` — the roster of a **cross-team** event you can see (333 added the `participations.event` m2o that makes it expressible, 334 the `events.open_roster` flag that keeps single-team events out) | participations (read only) |
...
| participations | `PARTICIPATION_VISIBLE` = `SAME_TEAM_AS_ME` ∪ `SAME_GAME_AS_ME` ∪ `EVENT_ROSTER_VISIBLE` | `MEMBER_PARTICIPATION_FIELDS` (all except the `last_*_edited_by` directus_users UUIDs — 2026-05-12 audit #12; incl. `auto_declined_by_game` since migration 261 and `event` since 333) | 033 / 271 / **333 + 334** |
...
| participations | read | `COACH_OR_TR_PARTICIPATION_READ` = `COACH_OR_TR_OF_PARTICIPATION` ∪ `{ activity_type: 'event' } _and { event: open_roster AND LEADER_EVENTS_VISIBLE }` — the whole roster of any event the leader can see | 026 / 271 / **333 + 334** |
