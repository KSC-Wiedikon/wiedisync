...
export default function MyRegistrationDocs() {
  ...
  const [preview, setPreview] = useState<{ doc: MyDoc; label: string } | null>(null)
  ...
  return (
    <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
      ...
      {/* Preview in place — PDFs render inline, so a member on a phone never
          leaves the app to look at their own ID scan. */}
      <FilePreviewDialog
        key={preview?.doc.field}
        open={!!preview}
        onOpenChange={(open) => { if (!open) setPreview(null) }}
        url={preview ? `${API_URL}/kscw/registration/my-docs/${preview.doc.field}` : null}
        label={preview?.label}
