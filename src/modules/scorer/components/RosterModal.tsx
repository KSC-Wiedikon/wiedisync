...
/**
 * Home-team roster for the assigned scorer (Schreiber), shown ±1h around the
 * game. Data comes from the time-gated /kscw/scorer/game/:id/roster endpoint —
 * the only place full DoB (incl. minors) is exposed, for match-sheet eligibility
 * checks. The component never holds roster data outside this modal.
 */
  ...
  return (
    <Modal open onClose={onClose} title={t('rosterTitle')} size="md" disableAutoFocus>
      ...
      {!loading && data && data.roster.length === 0 && (
        <p className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">{t('rosterEmpty')}</p>
      )}
      ...
      {!loading && data && data.roster.length > 0 && (
        <Table>
          ...
          <TableBody>
            {data.roster.map((r, i) => (
              <TableRow key={`${r.last_name}-${r.number ?? 'x'}-${i}`}>
