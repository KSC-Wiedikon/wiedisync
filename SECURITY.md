...
### 2026-07-05 — Deep 6-surface audit + remediation (sync-up/down, registration/signup, permissions)

Multi-agent audit (`/kscw-security-audit`, 6 parallel surface reviewers + adversarial per-finding verification, 32 agents) over everything since the 2026-07-02 pass — the signup-token rework, registration document enforcement, the ClubDesk sync-up create-set (Beitragskategorie/Eintritt/Gruppen/Status + migration 173), the clubdesk_id-keyed fill passes, and migrations 167–173. 26 raw findings → **17 confirmed after verification (2 high, 4 medium, rest low/info; 9 refuted)**. All remediated below. Migration **174** (schema-only); no permission ROW changed — the two HIGH + one MEDIUM are enforced by new BLOCKING kscw-hooks filters (the "Directus can't row-filter a CREATE" class the 2026-05-31 `assertCreateOwnership` guard already established for member-owned collections). Deploy: `db:deploy:*` (migrate 174 + setup-perms + smoke) → `ext:deploy:*`.

**High**
- **Coach/TR self-escalation to ANY team via unfiltered junction CREATE (HIGH #1/#2).** The LEADER policy grants `teams_coaches`/`teams_responsibles` CREATE, but Directus can't filter a CREATE and the only hooks were POST-insert ACTION hooks (which ATTACH the LEADER policy via `ensureLeaderAccess` — they GRANT, they can't reject). So a coach/TR of any single team could `POST /items/teams_coaches {teams_id:<any B>, members_id:<self>}` and gain write to team B's roster/games/fines/absences/events. New BLOCKING `teams_coaches/teams_responsibles.items.create` filter hooks (`actorLeadsTeam`) require the caller to already lead the target team (admin/system bypass). The misleading `setup-permissions.mjs` comment ("the role-sync hook gate it") corrected. This was the "junction-write tightening" backlogged in the shadow-coach-policy memory — now closed.

**Medium**
- **member_teams CREATE/UPDATE cross-team roster injection (MED #3).** `member_teams.items.delete` was team-scoped (2026-06-25) but create/update had no filter — a coach could add arbitrary members (or self) to teams they don't lead. New `member_teams.items.create` + `.update` filter hooks scope to teams the caller leads (no "self" bypass — self-add to an unrelated team IS the escalation).
- **finance_payouts.member ON DELETE CASCADE destroyed reimbursement history (MED #4).** Same audit-trail-destruction class migration 149 fixed for finance_payments/dunning. Migration **174** re-points to ON DELETE RESTRICT (a member with payout history can't be hard-deleted; the departed flow deactivates, so no live path changes).
- **Sync-up blank-risk gap on the member-flagged path (MED #5).** `/clubdesk-drift/flag` refuses members whose empty fields would blank the ClubDesk register, but the member-facing `/clubdesk-update` sets `clubdesk_push_pending` with no such check, so a profile edit clearing a field could reach `/clubdesk-member-sync/up`. `/up` now re-runs `computeClubdeskDrift` over the UPDATE set and drops blank-risk members (self-heal after a sync-down).
- **Registration approval auto-granted coach/TR from an applicant-controlled `rolle` (MED #6).** `rolle`+`team` are public-form fields; a `rolle=Trainer` auto-inserted `teams_coaches` on approval → materialized the LEADER policy (team leadership + PII). Approval no longer auto-creates the coach/TR junction — leadership is an explicit staff action (surfaced via an admin-visible `logWarning`); non-leader roles still roster as players.

**Low / Info**
- **Registration doc-attach brute-force (LOW #8).** The `/registration/:id/files` PENDING path authorized by the ~4-digit reference alone (overwrite-capable). Now requires the registration email as a mandatory second factor on BOTH pending+approved (feeding the existing lockout) + verifies the supplied file UUIDs actually exist in `directus_files`. Website belt-and-braces fallback updated to send the email (backward-compatible).
- **`/clubdesk-update` member-forgeable CSV (LOW #9).** The emailed "apply in ClubDesk" CSV + diff were built from client `current_data` — a member could forge AHV/Beitragskategorie/Anrede. Now built from the caller's authoritative DB row with the four ClubDesk-owned fields blanked, and the change diff whitelisted to member-editable fields. **Rate-limited** 5/hour/member (LOW #12).
- **Sync-up CREATE would silently duplicate a divergent-email contact (LOW #11).** `up-preview` now left-joins `clubdesk_export` on exact name and returns a `would_duplicate` flag so the modal warns before a CREATE push duplicates the contact.
- **Dispatcher create-stamp failure was silenced (LOW #10).** A committed CREATE set with a silenced `clubdesk_pushed_at` stamp failure would re-offer the contacts (duplicate on retry). The stamp now `fail_run`s loudly instead of `|| true`.
- **Importer data-integrity (LOW #13/#14/#15, INFO #16).** Birthdate email-match pass gained the last-name guard its siblings use (a shared family email + same first name could stamp a parent's adult DOB onto a minor → expose the minor on the public roster). AHV intake now applies the EAN-13 checksum to the already-dotted branch too (a dotted single-digit typo no longer permanently fills the legal-register column). clubdesk_id linker's first-name LIKE no longer collapses to match-all on a blank first_name. Stale "no UNIQUE constraint" comment corrected (migration 170's partial unique index is now the backstop).
- **slot_claims NULL-date bypass (LOW #7).** `trg_slot_claims_validate` did nothing for a NULL date (both guards no-op). Migration **174** rejects a NULL date up front (search_path stays pinned).
- **Refuted (9):** e.g. the `MEMBER_EDITABLE_FIELDS` whitelist already omits kscw_membership_active/shell/coach_approved_team (no privilege-flag-via-payload escalation); migrations 167–173 verified clean (idempotent, self-wrapped, correct partial-unique NULL semantics — the 56-member mass insert can't collide). Positive confirmation, not a defect.
...
| **LEADER (coach/TR) `/items/participations` read leaks cross-team (2026-07-05, MED, confirmed)** | **Open — needs scoped fix** | The `participations` read grant filter `member.member_teams.team.{coach,team_responsible}.members_id.user = $CURRENT_USER` is a 3-level M2M walk Directus does NOT enforce correctly — it OVER-returns (the deep-M2M-policy-walk class, here over-permissive not empty). Verified live on prod: TR 155 (coaches teams 6/81) reads participations of members 27/467 who are only on teams 11/82. Attendance/absence data (status + notes). NOT visible in the UI (coach views filter client-side by roster) — it's a direct-API leak. Fix requires narrowing the policy filter to one Directus enforces (or revoking the direct read and routing coach attendance through the existing scoped `POST /kscw/activities/:type/with-participations` endpoint) + e2e testing the coach roster modal — deliberately NOT bundled into the 2026-07-05 audit deploy to avoid an unverified coach-view regression. Same class likely affects the `absences` LEADER read — audit both together. Surfaced by `db:smoke` (the 2 long-standing coach-token failures). |
