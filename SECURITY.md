...
### 2026-09-15 — attack surface REMOVED: the messaging feature is gone (migration 364)

Not a finding — recorded so a future audit does not go looking for it. Team chat, DMs, group DMs, event activity-chats, reactions, message requests, member blocks and moderation reports were deleted wholesale (frontend module, `/kscw/messaging/*` ~30 routes, the retention cron, the acting-member deny list, broadcast's in-app channel, 7 tables, 8 trigger functions, 7 `members` columns, the Member-policy reads on `messages`/`message_reactions`/`message_requests`/`blocks`). Everything the 2026-07-13 realtime grants, the 2026-08-10 finding 8 (chat-poll content) and the 2026-09-01 "blocked while acting" note said about messaging is moot: the collections no longer exist. `polls` survives as team polls only (`polls.team` NOT NULL). The `system@kscw.ch` sentinel member (470) is now an ordinary, deletable row.

### 2026-09-01 — deliberate WIDENING (not a hardening): household guardians may act as the members they administer

Migrations 348 + 349. A parent gets one login and an explicit, revocable, audit-logged right to act as each of her children. **This is a real widening and the entry exists so a future audit reads it as a decision, not a hole.**

- **Mechanism.** `kscw-hooks/src/acting-member.js` runs at `middlewares.after` — verified ordering in the running 12.x image: `authenticate` :220 → `cache` :226 → `middlewares.after` :227 → endpoint router :273. It only ever **narrows** an identity Directus has already authenticated; it never bypasses `authenticate`, reads a credential, or elevates. The session cookie is never touched and no token is minted, so revocation is complete by construction — there is no minted session, device secret, or learned password to outlive it.
- **Four refusals, one opaque code.** No grant / target not active-or-managed-draft / target role is not exactly `Member` / the constructed accountability does not resolve to `{admin:false, app:true}` via Directus's own `fetchGlobalAccess`. All return the same `KSCW_ACTING_DENIED`; **distinct codes would be an enumeration oracle** letting any member map who holds an elevated role. A smoke check asserts two different unlinked targets refuse identically.
- **⚠⚠ Any throw returns 503 and never calls `next()`.** Falling through would run the request as the *guardian* while the client believes it is the child — a mis-attributed write, the one genuinely dangerous outcome.
- **Consent is structural, not procedural.** A grant may only target a member with no usable login (`members."user"` NULL, or a shadow user on `@managed.wiedisync.kscw.ch` with `status='draft'` and `password IS NULL`). **A member with her own real login can never be claimed** — verified on dev against two plain members. Two independent Directus gates make a shadow account unloginnable (`services/authentication.js` refuses `status !== 'active'`; `auth/drivers/local.js` throws on `!password`) and it consumes no licence seat. Additional refusals: guardian must hold a real login, target must not be staff (`teams_coaches`, `teams_responsibles`, `spielplaner_assignments`, `is_spielplaner`, or any role beyond the `user` baseline).
- **Blocked while acting** (endpoint-side, which is authoritative): password change, `set-password`, account deletion, household mutation (this closes co-guardian revocation abuse in a custody dispute), the iCal token mint (it mints a bearer that would outlive revocation), poll votes and scorer delegations. ⚠ The `DENY_PREFIXES` path list in the middleware is a coarse net only — Express matches `/kscw/*` case-insensitively, so `/kscw/Messaging/...` slips past it and is caught by the guard inside `requireAuth`.
- **⚠ `GUARDIAN_FORBIDDEN_CREATE`** exists because the swap otherwise **silently inverts** `assertCreateOwnership`: acting makes `editor.id === affectedMemberId`, so every "self only" collection would quietly become guardian-writable.
- **Blast radius, stated plainly.** Stealing a guardian's session yields write access to her children's member records (RSVPs, absences, profile, registration docs, invoices, forms). That is the price of the feature, bounded to the Member tier by the resolver cross-check. Stealing a managed child's account yields **nothing** — there is no credential.
- **Audit.** Both writers are stamped with `user_logs.acting_guardian`: the items-API one in `kscw-hooks/src/audit.js` (authoritative — raw knex, so the `user_logs.items.create` filter never sees those rows) and the custom-endpoint one in `activity-log.js`. Read-only acting sessions are logged too, throttled to one row per (guardian, target) per hour — that is what makes a parent merely *reading* a minor's data auditable. ⚠ Directus's own `directus_activity` still names the child with no acting column; `user_logs` at `/admin/audit-log` is the trail of record.

**Directus-upgrade checklist (added by this change):**
1. Re-verify `middlewares.after` still fires after `authenticate` and before `/items` and the endpoint router.
2. Re-verify the three resolver imports still resolve. ⚠ They are **internal** paths — `@directus/api/permissions/lib/fetch-roles-tree`, `.../modules/fetch-global-access/fetch-global-access`, `.../utils/create-default-accountability`. **`@directus/utils/node` does NOT exist in the container** (`@directus/` holds only `api` and `update-check`); the plan that specified it was wrong and the corrected specifiers were verified by importing them from the extensions directory itself. If an upgrade moves them the import throws, which refuses acting requests rather than over-granting — the safe direction, but it breaks the feature silently for guardians.
3. **`CACHE_ENABLED` must stay `false`.** Directus's cache middleware runs *before* the swap and keys `getCacheKey()` on the pre-swap user, so enabling it would cross-serve one household member's responses as another's. Pinned explicitly on dev; **must be pinned on prod in the same deploy.**
4. `CORS_ALLOWED_HEADERS` must include `X-KSCW-Acting-Member` and `CORS_EXPOSED_HEADERS` must include it too — the first or the switcher silently does nothing, the second or the client-side desync detector fails open. Both need a container **recreate**, not a restart.
...
| ~~Public-forms member-directory Flow `531dc3c2-…`~~ (#18, 2026-07-02) | **Resolved 2026-08-13 — scoped in Directus** (one residual, below) | The kscw-website autocomplete hit an unauthenticated Flow returning the **full roster**. Re-verified 2026-08-13 before the fix: **706 records, 70,864 bytes**, fields `id, name, functions, teams, sex, wiedisync_active` — i.e. **wider than this row originally recorded**, and ~2.5× the ~281 the public scope yields, so both the `website_visible` opt-out and the minor filter were being ignored. Cause: the `read_members` operation ran with **Permissions: Full Access**, which bypasses the Public policy entirely. Fixed by rewriting that operation's Query to mirror `setup-permissions.mjs` — `_and: [ website_visible _eq true, birthdate _lte $NOW(-18 years) ]` wrapped around the pre-existing `shell` `_or` — and dropping `sex` / `wiedisync_active` from `fields`. **Verified after: 284 records, `sex` null on all 284, `wiedisync_active` false on all 284** (the keys survive because a downstream merge builds the response object; they carry no data). ⚠ Permissions deliberately left on **Full Access**: the Public policy's field list is `['id','first_name','last_name','photo']`, and a single non-public field 403s the whole query — which would have silently emptied the member picker on both public forms via their `.catch`. Verify with: `curl -s https://directus.kscw.ch/flows/trigger/531dc3c2-64ec-4a7e-a989-da983d3530e4 \| jq 'length, .[0]'` |
| Member-directory Flow: internal `id` still returned + filter duplicated (2026-08-13) | Open — low, follow-up to #18 | Two residuals from the fix above. (1) The internal member `id` is still in the response: `mixed-tournament-form.js:112` and `volley-feedback-form.js:110` use it for the already-signed-up check (`CHECK_SIGNUP_FLOW?member_id=`), so it can only be dropped once that call takes an opaque lookup key instead. Scope is now opted-in adults only, so the id maps to a member who chose to be listed. (2) Because the operation stays on Full Access, the scope now exists **twice** — in the Flow's Query and in `setup-permissions.mjs` — and the two can drift apart silently. Durable fix is a dedicated non-admin policy for the Flow with exactly the fields these forms need; not done, as it is a larger change than the one that closed the exposure. |
...
Security-relevant note that earns its place in this file: `npm run db:deploy:<env>` runs `db:migrate → db:setup-perms → db:smoke`, and the **smoke test is the safety net for the silent-`Promise.all`-empty class** (the 4.4.4 / 2026-06-07 `fine_rules` incidents). `smoke-test.mjs` logs in as a non-admin Member and probes the read-rows whose loss silently empties `loadTeamContext` / HomePage — `teams_coaches`, `teams_responsibles`, `member_teams`, `fine_rules`, `rankings`, `forms` A re-granted-then-lost permission row turns the next deploy red the same minute it ships.
