...
import { buildPushCsv, CD_PUSH_CREATE_HEADERS, CD_KATEGORIE_MAP, mapKategorie, deriveGruppen } from '../clubdesk-update.js'
...
describe('buildPushCsv (create set)', () => {
  it('appends Beitragskategorie + Eintritt + Gruppen as the last three columns', () => {
    const csv = buildPushCsv([kacper], { create: true })
    const [header, row] = csv.trim().split('\n')
    expect(header).toBe(CD_PUSH_CREATE_HEADERS.join(';'))
    expect(header.endsWith('Beitragskategorie;Eintritt;Gruppen')).toBe(true)
  ...
  it('empty category / Eintritt / Gruppen yield empty cells (safe on a new contact)', () => {
    const row = buildPushCsv([{ ...kacper, beitragskategorie: null, eintritt: null, gruppen: '' }], { create: true })
      .trim().split('\n')[1]
    const cells = row.split(';')
    expect(cells[9]).toBe('')
