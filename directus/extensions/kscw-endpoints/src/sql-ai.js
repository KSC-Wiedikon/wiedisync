...
- Licence flags are eight BOOLEAN columns on members: \`scorer_vb\`, \`referee_vb\` (volleyball) and \`otr1_bb\`, \`otr2_bb\`, \`otn_bb\`, \`otn1_bb\`, \`otn2_bb\`, \`referee_bb\` (basketball). Query them directly (\`WHERE scorer_vb\`), not the legacy \`licences\` JSON column (which is being dropped). Count holders with \`COUNT(*) FILTER (WHERE scorer_vb)\`.
- OTN caveat: \`otn_bb\` is the COARSE LEGACY flag meaning "holds some OTN licence, level unknown" (ClubDesk could only ever express a single level-less "OTN"), while \`otn1_bb\` / \`otn2_bb\` are the precise Basketplan-sourced levels 1 and 2 — so for "who has an OTN licence" you MUST write \`WHERE otn_bb OR otn1_bb OR otn2_bb\`, never \`WHERE otn_bb\` alone (that misses every level holder) and never only the levels (that misses the legacy holders). Use \`otn1_bb\` / \`otn2_bb\` alone only when the user explicitly asks about a specific level.
