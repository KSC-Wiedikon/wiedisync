...
export function registerNominationPush(router, { database, logger }) {
  ...
  router.post('/games/:id/nomination-push', async (req, res) => {
    ...
    const game = await database('games').where('id', gameId).first('id', 'game_id', 'kscw_team', 'status')
    ...
    try {
      ...
      // Clear the previous failure so the UI shows "in progress", not a stale error.
      await database('games').where('id', gameId)
        .update({ vm_nomination_status: 'pending', vm_nomination_error: null })
