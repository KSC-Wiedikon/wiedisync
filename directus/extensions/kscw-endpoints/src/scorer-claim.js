...
const CLAIM_DEFS = {
  ...
  bb_24s_official:   { member: 'bb_24s_official',          duty: 'bb_24s_duty_team',            name: 'bb_24s_confirmed_by_name',            at: 'bb_24s_confirmed_at',            lic: ['otr2_bb', 'otn_bb'],  bbFallback: true },
}
  ...
  router.post('/games/:id/duty-claim', async (req, res) => {
    try {
      ...
      const member = await database('members').where('user', userId)
        .first('id', 'first_name', 'last_name', 'kscw_membership_active', 'scorer_vb', 'otr1_bb', 'otr2_bb', 'otn_bb')
