...
| `import-clubdesk-csv.mjs` | Transforms the ClubDesk CSV into SQL (`--emit-sql`) and feeds the `clubdesk_export` staging table. | `npm run db:clubdesk:import:dev` / `:prod`, and inside `db:clubdesk:sync:dev` / `:prod`. Also deployed to `/opt/clubdesk-sync/`. |
