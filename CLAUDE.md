...
- **2026-08-22** A multi-team event showed its whole roster to its whole audience (migrations 333 + 334, dev+prod).
- **2026-08-18** Events can un-invite the invited teams' guest players (migration 324, dev+prod).
- **2026-08-18** FIVB VIS now writes `members.transfer_status` — and the sync it rides on had been a season behind (no migration, dev+prod).
- **2026-08-15** The federation licence is itemised inside the dues rate — and kscw.ch had it backwards (migration 323, dev; prod pending approval, v1.96.0).
