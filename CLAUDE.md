...
- **2026-07-14** **KWI hall calendar is now two-way — wiedisync writes the home games** — the school's Google calendar was read-only to us (`.ics` → `hall_closures`); its *Heimspiele* half was hand-typed once a season and **2026/27 was empty** (70 VB home games missing). Backfilled all 70 + `gcal-push.js` reconciles nightly (create/update/delete), keyed on an invisible `extendedProperties` marker; BB joins automatically once Basketplan is scraped. Service-account auth (no npm dep). ⚠ **Dev runs `GCAL_PUSH_DRY_RUN=true`** — its DB is a prod clone, so a dev key would fight prod. ⚠ **A title prefix is not an identity**: skipping `BB `-titled events on import deleted 84 `hall_events` (the hall admin hand-types junior games that exist in no `games` row) → the guard keys on our own event ids. dev+prod.
- **2026-07-14** **Spielplanung — React #301 crash on the game calendar + blocked dates invisible (frontend-only)** — `useTeams` returned `data ?? []` (fresh array identity every render while loading), and `ManualGameModal` seeds form state **during render**, comparing deps with `Object.is` → never converged → "Too many re-renders". As an effect this was harmless; as a render-phase update it crashes. Fixed at source + hardened (compare arrays by joined key). Separately: the calendar's `closedDates` came **only** from `hall_closures`, so club-wide blocks (`scheduling_global_blocks`) never showed — now red, plus a **hard error** on a home game in a block (Week-view drag guarded too). Tabs now in the URL (`?view=`). ⚠ `spielplanung-dev.kscw.ch` serves the **prod** build, not the dev branch — use `dev.wiedisync-spielplanung.pages.dev`. dev+prod.
- **2026-07-14** **Coach match sheet (migration 211, dev+prod) + away Einsatzliste never read (dev+prod) + E2EE identity documents (migration 212, DEV ONLY)** — coaches/TRs get the match sheet and can adjust number/captain/libero **per game** (never back onto the club-wide `members`/`teams` columns); only add/drop can diverge from the Einsatzliste, and we never push to VM. The user's "coaches file one for away games too" exposed a **live bug**: the VM reader only ever read `nominationListTeamHome` and *asserted* the away half was always null — so **every away fixture silently fell back to RSVPs**, dropping un-RSVP'd nominated players in the away hall. Identity docs are genuinely E2EE (key made silently at login; **no escrow, no provisional keys** — coverage grows only as members log in). (Migration 206's "close" only *saves* the VM list — it stays editable until game start, so no conflict with the coach window.)
- **2026-07-14** **Basketplan cross-check — 8 members held a relative's licence number (migration 208)** — first authenticated read of `basketplan.ch` (club 166; creds vaulted at `services/basketplan`, login is browser-only — client-side password hashing, so no curl). Matching its 256 paid licences to `members` exposed 8 members carrying a **sibling's or parent's** `license_nr` — which is the exact join key to the VM Einsatzliste, so a wrong number = a player silently missing from a match sheet. Filled 125 licence numbers / 237 categories / 5 birthdates; addresses deliberately NOT imported (Basketplan is often the *worse* source). ⚠ `2xDU18` still points at the dead DU16 (7182), not DU18 B (7829) — deferred to October. dev+prod.
- **2026-07-14** **Officials licence dup + ClubDesk-only `VB SC` (migration 207)** — the Explorer column merged two sources holding the same fact (BB booleans + the ClubDesk string *derived from them* on push) → "OTR1, OTR1"; deduped. The dup exposed the real bug: a `VB SC` granted in ClubDesk never flowed back to `scorer_vb`, so 38 members were invisible to scorer assignment. Set-true sync-down rule in `import-clubdesk-csv.mjs` + backfill; dev+prod.
