...
- **Team pickers are ALWAYS grouped by sport**: any control that lets a user choose a team (single or multi, admin or member surface) MUST use `TeamPickerSingle` / `TeamPickerMulti` from `src/components/ui/TeamPicker.tsx` — they group Volleyball / Basketball with a VB/BB badge, fed from `teams.sport`. A native `<select>` is acceptable only inside a dense form and then MUST carry `<optgroup>` per sport (pattern: `IssueFinePickerModal.tsx`). Non-negotiable for any picker a superadmin / admin-mode user sees — that list holds both sports and team names alone don't say which (`Herren 2 H3` is basketball). Rule set 2026-09-15 after `/finance/team` shipped a flat list. Reference impl: `src/modules/finance/TeamFinancePage.tsx`.
...
- **2026-09-15** Season health `/admin/season-health` — superadmin whole-season diagnostic: 191 static-SQL checks in 8 sections + roster/team tables, VB/BB/club tabs, `npm run health:sql:dev` proves the registry on psql (no migration, dev; prod pending)
