# Security archive — KSCW (`wiedisync`)

Archived historical ledger split out of `SECURITY.md` to keep the live doc lean. This file is a **pure record** — the dated "Hardening completed" audit log of remediations already shipped. The current security posture (trust boundaries, open/accepted items, recurring gotchas, audit cadence, continuous verification) lives in `SECURITY.md`.
