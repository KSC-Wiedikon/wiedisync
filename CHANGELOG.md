...
- **v4.23.1 — Picture upload fix**: team-picture (RosterEditor) and member-photo (ProfileEditModal) uploads were silent no-ops — they passed a `FormData` body to `updateRecord()`, but the Directus SDK's `updateItem` does `JSON.stringify(body)` and `JSON.stringify(FormData) === '{}'`, so an empty PATCH "succeeded" and the toast said "Saved" while nothing persisted. Both now upload via `uploadFile()` (`POST /files`, real multipart) then set the FK with a plain-JSON update. New `useAuth().refreshUser()` re-fetches the member after a profile save so the new photo shows without a reload. Leave-team handlers (ProfilePage + TeamRequestModal) no longer swallow delete errors silently — they surface a `leaveTeamError` toast (×5 locales). Frontend-only; no migration/perms. (Two more spots share the old pattern and are NOT yet fixed: `TeamSponsorsEditor` sponsor logos and the `RecordEditModal` admin Daten-Explorer file fields.)
