...
- **Team absence calendar: multi-team selection, member de-dup, and two calendar filters.** The team absence view's team picker switched from single-select (`TeamFilter`) to the existing multi-select `TeamMultiSelect`, so admins and multi-team coaches can stack several teams at once (`AbsencesPage` `selectedTeam: string|null` → `selectedTeams: string[]`; `useTeamAbsences` already fetched `team: { _in }`). The day-overflow modal (`TeamAbsenceView`) now collapses a member who has **both** a one-off absence and a weekly unavailability on the same day into a single row labelled "Absent / Unavailable" (absence overrides unavailability, so the merged row opens the one-off entry); `MonthGrid`'s "N absent" pill counts **distinct members** to match. Two new calendar toggles: **"Show weekly unavailabilities"** (default on) and **"Show non-blocking absences"** (default off — absences flagged as not blocking game scheduling, e.g. injury/maternity, are hidden until requested). The non-blocking filter also applies to the absence list table. i18n ×5. Frontend-only.

## v4.19.0 — 2026-06-03

- **Volleymanager is now the source of truth for volleyball team names + leagues and referee licences.** The Volleymanager sync (`vm-sync-check.mjs`) moved from monthly to **weekly** (Mondays 04:00 UTC) and now also writes `teams.name` / `full_name` / `league` from VM — matched on `team_id` = `vb_<staticTeamIdentifier>`, **update-only** so app-only fields (colour, captain, photo, features) are never touched. It auto-corrects stale season-rollover clones: e.g. when two teams swap divisions the D1/D2 short labels + leagues follow the team that actually plays in each league. It also fetches club referees (`clubreferee`) and flags `members.referee_vb`, storing the managing association(s) in `sv_vm_check.referee_assoc` (migration **081** adds `is_referee` + `referee_assoc`). The **kscw-website** reads the same live data — team defs are keyed by the live team name (season-robust + swap-aware), the nav dropdowns and detail-page routing build from Directus, and a long-standing bug where the whole site silently fell back to static/stale leagues (a query for a non-existent `slug` field returned `FORBIDDEN`) is fixed; the detail chip now follows the live name so it can't mismatch the title. **Manual refresh:** `/status` gains a "Run now" button per scraper (Swiss Volley / Basketplan / Volleymanager / SVRZ / Google Calendar); VM + SVRZ run as fire-and-forget background jobs (`POST /kscw/admin/vm-sync` + `/svrz-sync`) recorded via the `sync_runs` heartbeat. Shipped to dev + prod.

## v4.18.0 — 2026-06-02

- **Per-member auto sign-in (auto-confirm RSVP) for trainings / games / events.** Profile gains three independent "Auto sign-in" toggles (migration **077**: `members.auto_confirm_{trainings,games,events}` booleans). When a member turns one on, they're auto-confirmed on every new activity of that type, and flipping it on backfills all their existing upcoming ones — already-answered and absence-declined activities are never touched (`NOT EXISTS`). Member opt-in is **OR-ed** with the team-level `training_auto_confirm` / `game_auto_confirm` setting, so a member can opt in even when their team default is off; there is no team-level equivalent for events (member opt-in only). `kscw-hooks`: `autoConfirmTraining`/`autoConfirmGame` now `JOIN members` and gate on `(teamOn OR m.auto_confirm_*)`; new `autoConfirmEvent` pass on `events.items.create` (whole-event mode only, eligibility = invited team ∪ individual invite ∪ club-wide); backfill runs on the `members.items.update` flag flip-on. Guests stay excluded from games (`guest_level = 0` + `trg_participations_guest_block`); per-training `excluded_guest_levels` still honoured. **Also closes a pre-existing gap:** SVRZ + Basketplan syncs insert games via raw knex and bypass `games.items.create`, so *neither* team nor per-member auto-confirm ever fired for synced games — a new idempotent `game-auto-confirm-sweep` runs at the end of each game sync to cover them. `setup-permissions`: 3 fields added to `MEMBER_EDITABLE_FIELDS` (self edit + read). i18n ×5. Verified end-to-end on dev (self-edit, backfill, absence-skip, future auto-confirm, team-off isolation).

## v4.17.0 — 2026-06-01

- **Absences: `blocking` flag for game scheduling.** New `absences.blocking` boolean (migration **076**, `NOT NULL DEFAULT true` — existing rows stay blocking) with an independent toggle in the absence modal ("Blocks game scheduling", default on, hint explaining injury/maternity exceptions). The `game-scheduling.js` endpoint ANDs `a.blocking IS NOT FALSE` into both absence queries (home-slot offering + opponent away-proposal strict/loose sets), so non-blocking absences no longer remove dates from the squad's Spielplanung availability. The per-player RSVP auto-decline cascade (kscw-hooks) is deliberately untouched — a non-blocking absence still marks that player as not attending their own games/trainings; `blocking` only governs *team-level* scheduling. Wired through `setup-schema.mjs`, `setup-permissions.mjs` (`MEMBER_ABSENCE_FIELDS` read scope), the `Absence` type, and i18n ×5. Weekly unavailabilities never affected scheduling, so the toggle lives only on the standard-absence modal.

## v4.16.2 — 2026-06-01

- **Calendar/Hallenplan: stale-team cleanup after the season rollover.** The `/admin/terminplanung/rollover-season` endpoint cloned each old team into a new-season team and copied the recurring hall-plan links via `cloneJunction('hall_slots_teams', …)` — but it *added* the new team's junction row without *removing* the old one, then archived the old team (`active=false`). Result after the 2026-06-01 rollover: 48/52 (prod) / 52/54 (dev) hall_slots were dual-linked `[archived team, active team]`, archived sorting first. The calendar resolves a slot's name/sport from `team[0]` and only loads active teams, so every recurring slot rendered grey + nameless and the VB/BB sport filter classified nothing. Three-part fix: (1) migration **075** deletes archived-team links from hall_slots where the slot keeps an active team (verified 0 slots reference only archived teams, so none is emptied; idempotent, data-only); (2) the rollover endpoint now *re-points* `hall_slots_teams` (move old→new, drop leftovers) instead of duplicating, so future rollovers don't recur — mirrors the existing `events_teams` re-point; (3) frontend hardening — `WeekSlotView`/`DaySlotView`/`SummaryView` resolve the first *identifiable* team instead of blindly `team[0]`, the VB/BB filter compares team ids as strings (was a silent number/string Set-lookup mismatch), and `SummaryView` now receives `teams` so it can resolve names at all. Dev: migration applied, kscw-endpoints deployed, verified clean.
...
- **Two new collections.** `fines` (the ledger: member, team, category, amount, status, optional activity link, audit snapshots) and `fine_rules` (per-team×category config: enabled, reset_window, tiers JSON, currency). Migration 069 ships both with idempotent `directus_collections` / `directus_fields` / `directus_relations` metadata so they appear in /admin. ON DELETE CASCADE on member/team FKs to follow the existing junction-cascade pattern (per `feedback_junction_cascade.md`).
- **Escalation engine.** Three new PL/pgSQL helpers — `kscw_current_season_start()` (mirrors `getCurrentSeason()` in `dateHelpers.ts`, Sep–Aug boundary), `kscw_fine_window_start(window, ts)` (dispatches the 5 reset_window enums), `kscw_compute_fine_amount(member, team, category)` (the engine: counts prior non-waived fines in the rule's window, looks up the matching tier by exact `offense`, then highest `offense_min ≤ N`, then last-tier fallback). Mirrored 1:1 in `src/hooks/useFines.ts` `computeFineAmount()` so the frontend pre-fill matches what the backend hook snapshots.
- **kscw-hooks `fines` block.** `filter('fines.items.create')` runs the engine, fills `amount` when null, snapshots `tier_offense` + `reset_window_at_issue`, resolves `issued_by` from accountability, forces `auto_issued = false`. `filter('fines.items.update')` blocks edits to `amount/category/reason/member/team/tier_offense/reset_window_at_issue` for non-admins — to correct a fine you waive + reissue (audit trail). `action('fines.items.create')` and `action('fines.items.update')` send push + in-app notifications to the recipient, localized through the existing 5-locale `push-i18n.js` bucket pattern. Daily 09:00 UTC `fines_reminder` cron rolls up open fines >14 days old into a single per-member push.
- **Permissions (setup-permissions.mjs).** MEMBER reads own fines + rules of teams they're on. COACH + TR get full CRUD scoped via `team.coach.members_id.user / team.team_responsible.members_id.user`. Vorstand gets read-all, Sport Admin gets full CRUD. Junction-walk pattern keeps the frontend safe from the M2M double-walk trap. PERMISSIONS.md updated in the same commit.
- **Frontend.** New `/fines` page with `<Table>` per the data-list rule (mobile compaction: status as colored badge, amount right-aligned tabular-nums, columns drop progressively at sm/md/lg). Member view shows the outstanding strip at the top. Leader view has a status + team filter and per-row actions (Mark as paid / Waive). `IssueFineModal` uses `useFineQuote()` to preview the engine's result inline ("2nd late sign-in this month → CHF 2"). `WaiveFineModal` requires a reason. `FinesSettings` accordion lives at the bottom of the team settings panel — per-category enable toggle, reset-window dropdown, tier editor with add/remove rows + "Nth and beyond" checkbox + live preview. ProfilePage strip links to `/fines` when the member has open fines. `FinesDashboardCard` adds a "Fines this month" tile to the coach dashboard with delta vs. last month.
- **Late-confirmation auto-prompt.** When a coach/TR confirms a member past `respond_by` on a single-team activity AND the team has `late_signin` enabled, `ParticipationRosterModal` pops `IssueFineModal` pre-filled with category=`late_signin` and the activity context. Skipped for multi-team events (ambiguous which team's rule applies) and for self-confirms.
- **i18n.** New `fines.ts` namespace × 5 locales with ~60 keys each. New `outstandingFines` + plural variants on `auth.ts`. New `fines` nav label.
