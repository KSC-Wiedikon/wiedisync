...
| `halls` | `id`, `name`, `address`, `city`, `courts`, `homologation`, `sv_hall_id`, `vm_hall_id` (VolleyManager hall uuid for game push — migration 104), `maps_url` | referenced by trainings/games/events/slots/closures | Sports hall master data. |
...
| `game_scheduling_seasons` | `id`, `season`, `status`, `spielsamstage` (json), `team_slot_config` (json), `gap_config` (jsonb), `svrz_season_uuid`, `season_opens`/`season_closes` (date, offer window — migration 108) | referenced by opponents/slots/bookings | Per-season Terminplanung config. |
...
| `game_scheduling_bookings` | `id`, `season`, `type`, `opponent`, `game`, `slot`, `svrz_game_id` (soft ref → svrz_games.svrz_persistence_id, per-fixture key — migration 105), `proposed_datetime_1..3`, `proposed_slot_1..3`, `confirmed_proposal`, `proposed_by_name`/`proposed_by_email` (migration 111), `vm_game_id`/`vm_push_status`/`vm_push_error`/`vm_pushed_at` (VolleyManager push — migration 104), `status` | `opponent`/`game`/`slot` (inferred) | Home-slot pick or away-proposal booking; keyed per SVRZ fixture (multi-game per opponent). |
...
| `svrz_spielplaner_contacts` | `id` (uuid), `svrz_persistence_id`, `season_uuid`, `team_identifier` (per-team key for team-responsible rows — migration 106), `club_*`, `person_*`, `contact_*`, `raw` (json) | standalone | SVRZ per-club / per-team scheduling contacts. |
| `vm_vb_spielplan_contact` | `id`, `FirstName`, `LastName`, `Email`, `Language` | standalone | Volleymanager scheduling-contact cache. |
| `scheduling_emails` | `id`, `message_id` (unique), `in_reply_to`, `references_ids`, `direction` (in/out), `folder`, `imap_uid`, `from_address`/`from_name`, `to_addresses`, `subject`, `body_text`/`body_html`, `read_at` | standalone (matched to opponents client-side by address) | Embedded Spielplanung mailbox for `volleyball@spielplanung.kscw.ch` (IMAP INBOX + Sent sync; attachment bytes streamed on demand, never stored) — migration 100. |
